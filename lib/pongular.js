...
var minErr = require('./minErr').minErr,
    HashMap = require('./apis').HashMap,
    utils = require('./utils'),
    assertNotHasOwnProperty = require('./utils').assertNotHasOwnProperty,
    glob = require('glob');

var pongular = exports.pongular = {
  injector: createInjector
};

var pongularModule = setupModuleLoader(pongular);
...
function setupModuleLoader(pongular) {
 ...
 *        {@link pongular.module}. The `ng` module must be explicitly added.
 ...
 * <pre>
 *   // create an injector
 *   var $injector = pongular.injector(['ng']);
 *
 ...
 * </pre>
 *
 * Sometimes you want to get access to the injector of a currently running Pongular app
 * from outside Pongular. Perhaps, you want to inject and compile some markup after the
 * application has been bootstrapped. You can do this using extra `injector()` added
 * to JQuery/jqLite elements. See {@link pongular.element}.
 *
 ...
 * it into the current PongularJS scope.
 *
 * <pre>
 * var $div = $('<div ng-controller="MyCtrl">{{content.label}}</div>');
 * $(document.body).append($div);
 *
 * pongular.element(document).injector().invoke(function($compile) {
 *   var scope = pongular.element($div).scope();
 *   $compile($div)(scope);
 * });
 * </pre>
 */
 ...
 *
 * <pre>
 *   var $injector = pongular.injector();
 ...
 * {@link pongular.Module}.
 *
 * An Pongular **service** is a singleton object created by a **service factory**.  These **service
 ...
 * module configuration function (see {@link pongular.Module#config}) but they can be overridden by
 * an Pongular
 ...
 * injected into a module configuration function (see {@link pongular.Module#config}) and it cannot
 * be overridden by an Pongular {@link AUTO.$provide#decorator decorator}.
 *
...
function createInjector(modulesToLoad) {
  ...
  utils.forEach(loadModules(modulesToLoad), function(fn) { instanceInjector.invoke(fn || noop); });
    ...
    utils.forEach(modulesToLoad, function(module) {
      ...
      try {
        if (utils.isString(module)) {
          moduleFn = pongularModule(module);
  ...
  function createInternalInjector(cache, factory) {
    ...
    function invoke(fn, self, locals){
      ...
      // http://jsperf.com/pongularjs-invoke-apply-vs-switch
      // #5388
    ...
    function instantiate(Type, locals) {
      ...
      return utils.isObject(returnedValue) || utils.isFunction(returnedValue) ? returnedValue : instance;
    }
